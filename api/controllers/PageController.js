...
module.exports = {
    ...
    showAdminPage: function (req, res) {
        ...
        User.findOne(req.session.me, function (err, user) {
            ...
            if (!user) {
                sails.log.warn('Сеанс относится к пользователю, который больше не существует, удалил ли пользователь, а затем попытался обновить страницу с открытой вкладкой, вошедшей в систему под этим пользователем?');
                return res.view('public/header', {layout: 'homepage', me: null});
            }
    ...
    showVideosPage: function (req, res) {
        ...
        User.findOne(req.session.me, function (err, user) {
            ...
            if (!user) {
                sails.log.warn('Session refers to a user who no longer exists- did you delete a user, then try to refresh the page with an open tab logged-in as that user?');
                return res.view('videos', {
                    me: null
                });
            }
